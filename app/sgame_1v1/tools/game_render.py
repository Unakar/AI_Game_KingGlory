...
# need pip install opencv-python
import cv2
import numpy as np
...
class GameRender:
    ...
    def __init__(self, frame_frequency=1, dump_path="./"):
        ...
        self.image_buffer =[np.zeros((self.WIDTH//self.SCALE,self.HEIGHT//self.SCALE, 3), np.uint8) for _ in range(self.max_frames)]
        self.cur_b_index = 0
        self.cur_game_id = "0"
    ...
    def draw_frame(self, req_pbs_in, frame_no):
        ...
        for id, req_pb in enumerate(req_pbs):
            player_id = req_pb.cmd_list[0].player_id
            camp = 0
            for state in req_pb.frame_state.hero_states:
                if state.actor_state.runtime_id == player_id:
                    camp = state.actor_state.camp
                    break
            player_ids.append(player_id)
            camps.append(camp)
            for state in req_pb.frame_state.npc_states:
                if (not full_info) or state.camp == camp:
                    self._draw_unit(state)

            for state in req_pb.frame_state.bullets:
                if (not full_info) or state.camp == camp:
        ...
        for id, req_pb in enumerate(req_pbs):
            ...
            camp = camps[id]
            for state in req_pb.frame_state.hero_states:
                if (not full_info) or state.actor_state.camp == camp:
                    self._draw_unit(state.actor_state)

        # draw info as text
        self._draw_info(req_pb, frame_no)
    ...
    def dump_one_frame(self, idx=None):
        ...
        cv2.imwrite(self.dump_path + file_name, frame, [int(cv2.IMWRITE_PNG_COMPRESSION), 0])
        print("dump frame {}/ {} at {}".format(idx, self.cur_b_index, self.dump_path+file_name))
    ...
    def _create_video_file(self):
        ...
        # (*'DVIX') or (*'X264')
        # if not working, pls install ffmepg: sudo apt-get install ffmepg
        cnt = str(random.randint(1,1000))
        file_name = "zpl_replay_{}_{}.avi".format(self.cur_game_id,cnt)
        # print("dump video {} at {}".format(self.cur_b_index, self.dump_path + file_name))
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        # fourcc = cv2.VideoWriter_fourcc(*'PIM1')
        # fourcc = cv2.VideoWriter_fourcc(*'X264')
        self.video_writer = cv2.VideoWriter(self.dump_path + file_name, fourcc, fps, self.video_shape)
    ...
    def _draw_unit(self, actor_state):
        ...
        if actor_state.max_hp > 0:
            color_factor =actor_state.hp*1. / actor_state.max_hp * (0.9)+0.1
        else:
            color_factor = 1
        color = self.rescale_color(self.CAMP_COLOR[camp], color_factor)

        # draw hero
        if actor_state.actor_type == 0:
            self._draw_circle(pos, color, self.UNIT_SIZE[0], self.UNIT_THICK[0])
            self._draw_line(pos, pos+forward, color, 1, 1)
        elif actor_state.actor_type == 1:
            self._draw_circle(pos, color, self.UNIT_SIZE[1], self.UNIT_THICK[1])
        elif actor_state.actor_type == 2:
            if self.ORGAN_RANGE.get(actor_state.sub_type) is not None:
                size = self.ORGAN_RANGE[actor_state.sub_type] // self.SCALE
    ...
    def _draw_element(self, e_type, pos_list, color=None, size=2, thickness=0):
        ...
        if e_type == "circle":
            ...
            cv2.line(self.image_buffer[self.cur_b_index], fpos, tpos, color, size, thickness)
    ...
    def _draw_text(self, str, pos, color=None, size=0.5, thickness=1):
        ...
        # print('text', pos)
        cv2.putText(self.image_buffer[self.cur_b_index], str, pos, font, size, color, thickness,
